...
语句以 `;`、换行或 `}` 结尾，三者均可；`;` 可省略，仅用于同行写多条语句。多行表达式需把运算符写在行尾续行，或靠未闭合的括号续行；行首的运算符 / `.` / `(` / `[` 不续接上一行，而是开启新语句。C 风格 `for(init;cond;inc)` 中的两个 `;` 是分隔符，仍必填。`for (let i = 0; ...)` 声明块级局部变量。赋值（`x = v`、`x += v`、`x++`）只能操作已声明变量；给未声明名赋值报错。`for-of` 的 `const` 循环变量只读，`let` 与省略写法可写。
